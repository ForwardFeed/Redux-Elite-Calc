...
export function getMoveEffectiveness(
  ...
  if (move.type == 'Psychic' && target.hasAbility('Gifted Mind')) {
    ...
    return (
      ...
      gen.types.get('Ground' as ID)!.effectiveness[type]!
    );
  } else {
...
export function checkIntimidate(gen: Generation, source: Pokemon, target: Pokemon) {
  const blocked =
    target.hasAbility('Clear Body', 'White Smoke', 'Hyper Cutter',
...
export function checkScare(gen: Generation, source: Pokemon, target: Pokemon) {
  const blocked =
    target.hasAbility('Clear Body', 'White Smoke', 'Hyper Cutter',
      'Full Metal Body', 'Discipline', 'Overwhelm') ||
    (gen.num >= 8 && target.hasAbility('Inner Focus', 'Own Tempo', 'Oblivious', 'Scrappy')) ||
