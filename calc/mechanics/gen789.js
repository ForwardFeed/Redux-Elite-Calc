...
function calculateSMSSSV(gen, attacker, defender, move, field, defenderFriend) {
    ...
    if (defender.types.length === 3) {
        var type3Effectiveness = defender.types[1]
            ? (0, util_2.getMoveEffectiveness)(gen, move, defender.types[1], isGhostRevealed, field.isGravity, isRingTarget)
            : 1;
    ...
    var contribution = calcContribution(gen, attacker, move);
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox')) ||
        ...
        var type3Effectiveness = types[2]
            ? (0, util_2.getMoveEffectiveness)(gen, move, types[2], isGhostRevealed, field.isGravity, isRingTarget)
            : 1;
...
function calculateAtModsSMSSSV(gen, attacker, defender, move, field, desc) {
    ...
    if (attacker.hasAbility('Sage') && move.category === 'Special') {
        atMods.push(6144);
    }
...
function calcContribution(gen, attacker, move) {
