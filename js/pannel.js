class Panel{
    ...
    showFormes(forme) {
        var formes = this.pokemon.otherFormes.slice();
        formes.unshift(forme);
    
        var defaultForme = formes.indexOf(this.pokemon.species);
    ...
    setPanel(){
        ...
        this.level = poke.lvl || 1
        var baseForme
        if (poke.baseSpecies && poke.baseSpecies !== poke.species) {
		    baseForme = pokedex[poke.baseSpecies];
        }
        if (poke.otherFormes) {
            this.showFormes(poke.species);
        } else if (baseForme && baseForme.otherFormes) {
            this.showFormes(poke.baseSpecies);
        } else {
        ...
        this.ability = poke.abilities[poke.ability] || poke.ability || poke.abilities[0] || ""
        this.abilityOn = false
        this.item = poke.heldItem || poke.item || ""
        this.status = "Healthy"
        this.stats.percentHP = 100
        for (const index in poke.moves) {
            const move = poke.moves[index]
            if (!move) continue
